    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (player.getLevel() < exped.getMinLevel() || player.getLevel() > exped.getMaxLevel()) { //Don't fit requirement, thanks Conrad
                ...
                cm.dispose();
            } else if (expedition == null) { //Start an expedition 暴力熊
            ...
            if (selection == 1) {
                ...
                if(cm.haveItem(2022035,1)){
                    ...
                    //    cm.sendOk("远征队长必须持有一个 #b#t" + expedItem + "##k 才能入场挑战" + expedBoss + "！");
                    cm.sendOk("你的远征队长必须拥有#v2022035#才可进入");
                    cm.dispose();
                    return;
                }
            ...
            if (selection == 1) {
                ...
                cm.gainItem(expedItem, 2); //itemid4032246
                status = 4;
            } else if (selection == 3) {
