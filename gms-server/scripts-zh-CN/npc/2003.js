...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (sel == 0) {
            cm.sendNext("我告诉你基本移动方法。用#bleft, right arrow#k可以在平地或斜坡上行走，用#bAlt#k可以跳跃。有些鞋子可以增加你的移动速度或跳跃能力。");
        } else if (sel == 1) {
            cm.sendNext("我告诉你打退怪物的方法。每只怪物都有自己的体力，用物理攻击或魔法攻击都可以打退怪物。但是有些怪物很厉害，打退它们很不容易");
        } else if (sel == 2) {
            cm.sendNext("我告诉你捡取物品的方法。打退怪物后战利品会在掉在地上，这时候站在物品前按#bZ#k或#b小键盘上的0#k就可以拣取物品。");
        } else if (sel == 3) {
