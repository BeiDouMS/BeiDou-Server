...
public final class FakePlayerRandomizer {
     ...
     * 装备池（按位置分类）
     * <p>
     * 位置: 1=帽子, 2=脸饰, 3=眼饰, 4=耳环, 5=上衣, 6=裤子, 7=鞋子, 8=手套, 9=盾牌/披风
     * </p>
     */
    private static final int[][] EQUIP_POOLS = {
            {1002000, 1002001, 1002002, 1002003, 1002004, 1002005, 1002006, 1002007},  // 帽子
            {1012000, 1012001, 1012002, 1012003},  // 脸饰
            {1022000, 1022001, 1022002, 1022003},  // 眼饰
            {1032000, 1032001, 1032002, 1032003},  // 耳环
            {1040000, 1040001, 1040002, 1040003, 1040004, 1040005},  // 上衣
            {1050000, 1050001, 1050002, 1050003},  // 套装（连体衣）
            {1060000, 1060001, 1060002, 1060003},  // 裤子/裙子
            {1072000, 1072001, 1072002, 1072003, 1072004, 1072005},  // 鞋子
            {1082000, 1082001, 1082002, 1082003},  // 手套
            {1092000, 1092001, 1092002, 1092003},  // 盾牌
            {1102000, 1102001, 1102002, 1102003}   // 披风
    };
    ...
    private static final int[][] WEAPON_POOLS = {
            {1302000, 1302001, 1302002, 1302003},  // 单手剑
            {1312000, 1312001, 1312002},  // 单手斧
            {1322000, 1322001, 1322002},  // 单手钝器
            {1332000, 1332001, 1332002},  // 短刀
            {1342000, 1342001},  // 双刀
            {1452000, 1452001, 1452002},  // 弓
            {1462000, 1462001, 1462002},  // 弩
            {1432000, 1432001, 1432002},  // 矛
            {1442000, 1442001, 1442002},  // 枪
            {1472000, 1472001, 1472002},  // 拳套
            {1482000, 1482001, 1482002},  // 指节
            {1492000, 1492001, 1492002}   // 枪（火枪手）
    };
    ...
    private static final String[] GUILD_NAMES = {
            "冒险小队", "星辰公会", "暗夜骑士团", "光明圣殿", "龙之传人",
            "英雄联盟", "梦幻战队", "风暴骑士", "烈焰军团", "冰霜之刃",
            "无名剑客", "天涯海角", "王者归来", "铁血丹心", "义薄云天"
    };
    ...
    public static FakePlayer generateRandom() {
        ...
        if (rand.nextDouble() < 0.7) {
            ...
            player.setGuildName(GUILD_NAMES[rand.nextInt(GUILD_NAMES.length)]);
        }
     ...
     *
     * @param job 职业
     * @return 转职阶段 (0=一转, 1=二转, 2=三转, 3=四转)
     */
    private static int getJobTier(Job job) {
        ...
        if (jobId < 600) {
            return Math.min(jobId % 100 / 10, 3);
        }
        // 冒险家四转职业
