    ...
    if (actionx["Mental"]) {
        if (status == 0) {
            ...
            if (Math.floor(cm.getJobId() / 10) == 41) {
                cm.sendNext("你已经正式被封为#b隐士#k。这本技能书为隐士引入了一系列新的攻击技能，利用影子进行复制和替代，包括#b影子金币#k（用金币代替MP，并根据投掷的金币数量对怪物造成伤害）和#b模仿猫#k（创建一个模仿每一个动作的影子，使隐士能够像两个隐士一样攻击怪物）。利用这些技能来对抗以前可能难以征服的怪物。");
            } else {
                cm.sendNext("你已经正式被任命为#b首领强盗#k。技能书中的新技能之一是#b强盗团#k，你可以召唤其他强盗一起攻击多个怪物。首领强盗还可以利用游戏币进行多种操作，从攻击怪物（#b游戏币爆炸#k，将地面上的游戏币引爆）到自我防御（#b游戏币守护#k，减少武器伤害）。");
            }
