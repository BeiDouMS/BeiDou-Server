...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getGuildId() < 1 || cm.getPlayer().getGuildRank() != 1) {
            cm.sendNext("你好！我是#bLenario#k。只有公会会长才能尝试组建公会联盟。");
            cm.dispose();
            return;
        }
        ...
        choice = selection;
        if (selection == 0) {
            cm.sendNext("公会联盟就像它的名字一样，是由多个公会组成的超级团体。我负责管理这些公会联盟。");
            cm.dispose();
        } else if (selection == 1) {
            cm.sendNext("要创建公会联盟，需要两位且仅限两位公会会长组成一个队伍，并且必须在同一个频道的房间内。该队伍的队长将被指定为公会联盟的主人。\r\n最初，只有两个公会可以加入新的联盟，但随着时间的推移，你可以通过在特定时机与我交谈并投资一定费用来扩大联盟的容量。");
            cm.dispose();
        } else if (selection == 2) {
            if (!cm.isLeader()) {
                cm.sendNext("如果你想组建公会联盟，请告诉你的队伍领袖与我交谈。他/她将被指定为公会联盟的领袖。");
                cm.dispose();
                return;
            }
            if (cm.getPlayer().getGuild().getAllianceId() > 0) {
                cm.sendOk("当你的公会已经注册在其他公会联盟中时，你无法创建公会联盟。");
                cm.dispose();
                return;
            }

            cm.sendYesNo("哦，你对组建公会联盟感兴趣吗？目前这项操作的费用是 #b" + allianceCost + " 冒险币#k。");
        } else if (selection == 3) {
            if (cm.getPlayer().getMGC() == null) {
                cm.sendOk("如果你没有公会，就无法扩展公会联盟。");
                cm.dispose();
                return;
            }
            ...
            if (rank == 1) {
                cm.sendYesNo("你想要增加你的公会联盟 #rone guild#k 的位置吗？这个手续的费用是 #b" + increaseCost + " 冒险币#k。");
            } else {
                cm.sendNext("只有公会联盟会长才能扩大公会联盟中的公会数量。");
                cm.dispose();
            }
        } else if (selection == 4) {
            if (cm.getPlayer().getMGC() == null) {
                cm.sendOk("如果你没有公会联盟，你就无法解散公会联盟。");
                cm.dispose();
                return;
            }
