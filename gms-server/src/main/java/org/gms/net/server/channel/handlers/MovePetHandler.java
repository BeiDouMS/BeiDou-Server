...
//        Point startPos = StreamUtil.readShortPoint(slea);
    ...
    private final boolean shouldPickupItem(Character player, byte slot, MapItem mapitem) {
        if (mapitem == null || mapitem.isPickedUp()) {
            return false;
        }
        if (mapitem.getMeso() > 0) {
            if (!player.isEquippedMesoMagnet(slot)) {
                return false;
            }

            if (player.isEquippedPetItemIgnore(slot)) {
                final Set<Integer> petIgnore = player.getExcludedItems();
                return petIgnore.isEmpty() || !petIgnore.contains(Integer.MAX_VALUE);
            }
        } else {
            if (!player.isEquippedItemPouch(slot)) {
                return false;
            }
    ...
    public void itemVac(Character player,byte slot) {
        ...
        for (MapObject item : list) {  // 遍历宠物身边一定范围内的物品列表
            ...
            if (shouldPickupItem(player, slot, mapItem) && (mapItem.getOwnerId() == player.getId()
