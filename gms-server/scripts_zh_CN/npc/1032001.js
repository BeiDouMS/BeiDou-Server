...
function start() {
    ...
    if (parseInt(cm.getJobId() / 100) == jobType && cm.canSpawnPlayerNpc(GameConstants.getHallOfFameMapid(cm.getJob()))) {
        ...
        if (cm.getJobId() == 0) {
            ...
            cm.sendNext("你来了。几天前，奥西里亚的#b#p2020009##k跟我谈到了你。我看到你对成为魔法师的第三次职业转职感兴趣。为了实现这个目标，我需要测试你的实力，看看你是否配得上这个晋升。在维多利亚岛的一个邪恶深林中有一个开口，会带你通往一个秘密通道。一旦进入，你将面对我的一个克隆。你的任务是打败他，并带着#b#t4031059##k回来。");
    ...
    if (actionx["1stJob"]) {
        ...
        if (status == 0) {
            ...
            if (!cm.haveItem(4031012)) {
                ...
                if (selection < 3) {
                    if (selection == 0) {
                        cm.sendNext("掌握#r火/毒系魔法#k的魔法师。\r\n\r\n#b巫师#k是一个活跃的职业，能够造成魔法元素伤害。这些技能使他们在面对对其元素弱点的敌人时具有显著优势。通过他们的技能#r冥想#k和#r减速#k，#b巫师#k可以增加他们的魔法攻击并减少对手的移动能力。#b火/毒巫师#k拥有强大的火焰箭攻击和毒素攻击。");    //f/p mage
                    } else if (selection == 1) {
                        cm.sendNext("掌握#r冰/雷系魔法#k的魔法师。\r\n\r\n#b巫师#k是一个活跃的职业，能够造成魔法元素伤害。这些技能使他们在面对对其元素弱点的敌人时具有显著优势。通过他们的技能#r冥想#k和#r减速#k，#b巫师#k可以增加他们的魔法攻击力并减少对手的移动能力。#b冰/雷巫师#k拥有冰冻攻击和闪电攻击。");    //i/l mage
                    } else {
            ...
            cm.sendNext("好的，从现在开始你就是#b法师（火/毒）#k了。法师和巫师是聪明的一群，拥有令人难以置信的魔法能力，能够轻松地穿透怪物的心灵和心理结构……请每天都训练自己。我会帮助你变得比你现在更强大。");
            if (cm.getJobId() != job) {
