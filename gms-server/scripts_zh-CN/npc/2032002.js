    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (!eim.isEventCleared()) {
                ...
                if (selection == 0) {
                    cm.sendNext("为了揭示扎昆的力量，你需要重新制造它的核心。在这个地牢的某个地方隐藏着一块#b\"火焰矿石\"#k，这是制作核心所需的材料之一。找到它，然后带给我。\r\n哦，你能帮我一个忙吗？这附近的石头下面也有一些#b纸质文件#k。如果你能找到30份，我会奖励你的努力。");
                    cm.dispose();

                } else if (selection == 1) {
                    if (!cm.isEventLeader()) {
                        cm.sendNext("请让你们的领袖把材料带给我，以完成这个考验。");
                        cm.dispose();
                        return;
                    }

                    if (!cm.haveItem(4001018)) { //fire ore
                        cm.sendNext("请带上#b火焰矿石#k。");
                        cm.dispose();
                    } else {
                        gotAllDocs = cm.haveItem(4001015, 30);
                        if (!gotAllDocs) { //documents
                            cm.sendYesNo("所以，你带了火之矿石来了？在这种情况下，我可以给你和你的每个队员一块，这应该足够制作扎昆的核心。确保你的整个队伍在继续之前有足够的背包空间。");
                        } else {
                            cm.sendYesNo("所以，你带来了火之矿石和文件吗？在这种情况下，我可以给你和你的每个队员一块，这应该足够制作扎昆的核心了。另外，既然你带来了文件，我还可以给你一个特殊物品，可以随时带你到矿井入口。在继续之前，请确保你的整个队伍的背包有足够的空间。");
                        }
                    }
                } else if (selection == 2) {
                    cm.sendYesNo("你确定要退出吗？如果你是队伍的队长，你的队伍也将从矿坑中移除。");
                }
            } else {
