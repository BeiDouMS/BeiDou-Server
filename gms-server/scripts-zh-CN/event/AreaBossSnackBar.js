...
// 地图 ID (BOSS 生成的地图)
const MapID = 105090310;

// BOSS ID (要生成的怪物 ID)
const BossID = 8220009;
...
function getRandomSpawnPoint() {
    var rnd = SpawnPositions[Math.floor(Math.random() * SpawnPositions.length)];
    return new Point(rnd[0], rnd[1]);
}

// BOSS 出现时的全服公告内容
const BossNotice = "正缓缓在幽僻的荒郊野径旁支起招牌，蒸腾的热气在空荡的山路上显得格外扎眼。‌";
...
// 事件实例名称 (自动获取，无需修改)
const EventName = "AreaBossSnackBar";
...
function spawnBoss(eim, map) {
    try {
        ...
        if (log) log.info(`[野外BOSS] ${EventName} 已在频道 ${channel} 生成 ${BossName}(${BossID}) @ (${spawnPoint.x}, ${spawnPoint.y})`);
